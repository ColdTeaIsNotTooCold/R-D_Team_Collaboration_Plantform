...
此命令通过读取项目上下文文档和理解代码库结构，为新代理会话加载基本上下文。
...
**如果关键文件缺失：**
- `project-overview.md` 缺失：尝试从README.md理解
- `tech-context.md` 缺失：直接分析package.json/requirements.txt
- `progress.md` 缺失：检查最近的git提交以了解状态
...
🔍 项目理解：
  ...
  - 类型：{project_type}
  - 语言：{primary_language}
  - 状态：{来自progress.md的current_status}
  - 分支：{git_branch}
